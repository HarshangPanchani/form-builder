...
class JSONSchemaToDatabase:
    ...
    def _traverse(self, schema, tree, path=tuple(), table='root', parent=None, comment=None, json_path=tuple()):
        ...
        if special_keys:
            ...
            if tree['type'] == 'null':
                res = {}
            elif tree['type'] not in ['string', 'boolean', 'number', 'integer','array']:
    ...
    def create_tables(self, con,app_name,defName):
        '''Creates tables

        :param con: psycopg2 connection object
        '''
        postgres_types = {'boolean': 'bool','array':'text[]', 'number': 'float', 'string': 'text', 'enum': 'text', 'integer': 'bigint', 'timestamp': 'timestamptz', 'date': 'date', 'link': 'integer'}
        with con.cursor() as cursor:
            ...
            for table, columns in self._table_columns.items():
                ...
                if identifier==0:
                    create_q = 'create table IF NOT EXISTS %s (id %s, "%s" %s not null, "%s" text not null, %s unique ("%s", "%s"), unique (id))' % \
                    ...
                    create_q = 'create table IF NOT EXISTS %s (id %s, "%s" %s not null, "%s" text not null, %s unique ("%s", "%s"), unique (id))' % \
                               (self._postgres_table_name(f"ts_{app_name}_form_data_{defName}_{table}"), id_data_type, self._item_col_name, postgres_types[self._item_col_type], self._prefix_col_name,
                                ''.join('"%s" %s, ' % (c, postgres_types[t]) for c, t in zip(columns, types)),
                                self._item_col_name, self._prefix_col_name)
                    self._execute(cursor, create_q)
                    if table in self._table_comments:
        ...
        if not mutate:
            ...
            with con.cursor() as cursor:
                ...
                for table, data in data_by_table.items():
                    ...
                    self._execute(cursor, b'insert into %s %s values %s' % (self._postgres_table_name(table).encode(), cols.encode(), args), query_ok_to_print=True)
                    # print(b'insert into %s %s values %s' % (self._postgres_table_name(table).encode(), cols.encode(), args))

    def update_items(self, con, items,row_id=None, extra_items={}, mutate=True, count=False):
        ...
        if not mutate:
            ...
            with con.cursor() as cursor:
                ...
                for table, data in data_by_table.items():
                    ...
                    final = b'update %s set ' % (self._postgres_table_name(table).encode())
                    for i in range(len(cols)):
                        final += ('"'+cols[i]+'"').encode() + b'=' + cursor.mogrify('%s',[data[i]]) + b','
                    temp = b' where id = %s' % (cursor.mogrify('%s',[row_id]))
                    final = final[:-1] + temp
                    self._execute(cursor, final, query_ok_to_print=False)
